                            ...
                            swtpg::MessageRegistersInduction* __restrict__ induction_registers)
{
  ...
  for (size_t iframe = 0; iframe < swtpg::FRAMES_PER_MSG - 1; ++iframe) {
    ...
    // Same for induction registers
    for (size_t iblock = 0; iblock < swtpg::INDUCTION_REGISTERS_PER_FRAME ; ++iblock) {
      induction_registers->set_ymm(iframe + iblock * swtpg::FRAMES_PER_MSG, swtpg::unpack_one_register(frame->adc_words+7*(iblock+swtpg::COLLECTION_REGISTERS_PER_FRAME)));
    }
  }
}
