...
const constexpr std::size_t kExtendedTriggerPrimitive = sizeof(trgdataformats::ExtendedTriggerPrimitive);
...
struct ExtendedTriggerPrimitiveTypeAdapter : public TriggerPrimitiveTypeAdapter
{
  using FrameType = ExtendedTriggerPrimitiveTypeAdapter;
  // data
  // data
  trgdataformats::ExtendedTriggerPrimitive tp;
  // comparable based on start timestamp
  bool operator<(const ExtendedTriggerPrimitiveTypeAdapter& other) const
  {
    return std::tie(this->tp.time_start, this->tp.channel) < std::tie(other.tp.time_start, other.tp.channel);
  }
  ...
  size_t get_payload_size() { return kExtendedTriggerPrimitive; }

  size_t get_num_frames() { return 1; }

  size_t get_frame_size() { return kExtendedTriggerPrimitive; }
