...
protected:
  ...
  /**
   * Pipeline Stage 2.: Check WIB2 headers for error flags
   * */
  ...
  {
    constexpr int clocksPerTPCTick = types::WIB2_SUPERCHUNK_STRUCT::expected_tick_difference;
    ...
    while (*primfind_it != swtpg::MAGIC) {
      ...
      for (int i = 0; i < 16; ++i) {
        if (hit_charge[i] && chan[i] != swtpg::MAGIC) {
          ...
          trigprim.detid =
            m_link; // TODO: convert crate/slot/fiber to GeoID Roland Sipos rsipos@cern.ch July-22-2021
