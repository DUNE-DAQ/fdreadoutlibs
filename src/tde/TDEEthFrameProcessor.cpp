...
TDEEthFrameProcessor::TDEEthFrameProcessor(std::unique_ptr<datahandlinglibs::FrameErrorRegistry>& error_registry, bool processing_enabled)
  : TaskRawDataProcessorModel<types::TDEEthTypeAdapter>(error_registry, processing_enabled)
{
}
...
TDEEthFrameProcessor::stop(const appfwk::DAQModule::CommandData_t& args)
{
  ...
  if (m_post_processing_enabled) {
    ...
    m_tp_generator->configure(m_tpg_configs, m_channel_plane_numbers, types::TDEEthTypeAdapter::samples_tick_difference);
  }
}
...
TDEEthFrameProcessor::conf(const appmodel::DataHandlerModule* conf)
{
  ...
  if (dp != nullptr) {
    ...
    if (proc_conf != nullptr && m_post_processing_enabled) {
      ...
      inherited::add_postprocess_task(std::bind(&TDEEthFrameProcessor::find_hits, this, std::placeholders::_1));
    }
  }
  inherited::conf(conf);
}
...
TDEEthFrameProcessor::sequence_check(frameptr fp)
{
  ...
  if (m_emulator_mode) {  
    ...
    auto wf = reinterpret_cast<tdeframeptr>(((uint8_t*)fp));            // NOLINT
    for (unsigned int i = 0; i < fp->get_num_frames(); ++i) {           // NOLINT(build/unsigned)
      //auto wfh = const_cast<dunedaq::fddetdataformats::TDEEthFrame*>(wf->header());
...
TDEEthFrameProcessor::timestamp_check(frameptr fp)
{
  ...
  if (inherited::m_emulator_mode) {                                     // emulate perfectly incrementing timestamp
    ...
    for (unsigned int i = 0; i < fp->get_num_frames(); ++i) {           // NOLINT(build/unsigned)
      //auto wfh = const_cast<dunedaq::fddetdataformats::TDEEthFrame*>(tf->header());
      tf->daq_header.crate_id = m_crate_id;
      tf->daq_header.slot_id = m_slot_id;
  ...
  // Check timestamp
  if (m_previous_ts > 0 &&
      m_current_ts - m_previous_ts != tdeeth_frame_tick_difference) [[unlikely]] {
    ++m_ts_error_ctr;
    if (m_first_ts_missmatch) { // log once
      ...
      if (!m_ts_error_state) {
        m_error_registry->add_error("Timestamp jump", datahandlinglibs::FrameErrorRegistry::ErrorInterval(m_previous_ts + tdeeth_frame_tick_difference, m_current_ts));
 ...
 * */
void
TDEEthFrameProcessor::find_hits(constframeptr fp)
{
  size_t nhits = 0;
  if (!fp)
    return;
  auto wfptr = reinterpret_cast<dunedaq::fddetdataformats::TDEEthFrame*>((uint8_t*)fp); // NOLINT
