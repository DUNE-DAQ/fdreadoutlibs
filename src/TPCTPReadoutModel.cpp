...
void TPCTPReadoutModel::push_reordered_tp(trgdataformats::TriggerPrimitive& payload)
{
...
TPCTPReadoutModel::run_consume()
{
  ...
  while (m_run_marker.load()) {
    ...
    if (opt_payload) {
      if (first_cycle) {
        start_win_ts = opt_payload.value().time_start;
        first_cycle = false;
      }
      m_tps_map[std::pair<trgdataformats::timestamp_t, trgdataformats::channel_t>
      (opt_payload.value().time_start, opt_payload.value().channel)] = opt_payload.value();
